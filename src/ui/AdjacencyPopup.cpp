...
protected:
    ...
    bool init(int colors) {
        ...
        CCMenuItemToggler* toggler = CCMenuItemToggler::createWithSize("GJ_checkOff_001.png", "GJ_checkOn_001.png", this,  menu_selector(AdjacencyPopup::onGradientToggle), 0.5f);
        toggler->setPosition({100, 100});
        toggler->toggle(settings.isGradientActive());
        m_closeBtn->getParent()->addChildAtPosition(toggler, Anchor::Top, ccp(-50.f, -40.f));
        ...
        m_slider->setValue(0.5f); // valor inicial (0.0 - 1.0)
        m_slider->setScale(0.6f);
        m_slider->setAnchorPoint(ccp(0.f, 0.f));
        m_slider->setRotation(-90.f);
        m_slider->m_enabled = false; // deshabilitado por defecto
        ...
        m_mainLayer->addChildAtPosition(index_x, Anchor::Center, ccp(0.f, (m_grid->getContentSize().height / 2 + 10.f) * m_grid->getScale() - 30.f));
        ...
        for (int i = 0; i < m_colors; i++) {
            ...
            for (int j = 0; j < btnsPerRow; j++) {     
                ...
                CCLabelBMFont* label = CCLabelBMFont::create(numToString(settings.getAdjacency(tag / m_colors, (tag + tag / m_colors) % m_colors + 1)).c_str(), SpriteBuilder::bigFontName);
                label->setScale(0.3f);
